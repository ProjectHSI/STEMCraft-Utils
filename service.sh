...
B2_PATH=b2
B2_ACC_ID="secret-id"
B2_APP_KEY="secret-key"
B2_BUCKET_NAME="secret-bucket"
...
backup() {
    ...
    then
        if is_server_running; then
            echo "Saving server files and disabling server save"
        ...
        if is_server_running; then
            echo "Enabling server save"
            mc_command "save-on"
            mc_command "save-all"
            mc_command "tellraw @a {\"text\":\"[SERVER] Storing backups...\",\"color\":\"gold\"}"
        fi

        ${B2_PATH} account authorize "$B2_ACC_ID" "$B2_APP_KEY" >/dev/null

        cd "${BACKUP_DIR}" || exit

        for f in *; do
            echo "Uploading $f to B2"
            ${B2_PATH} file upload --no-progress "$B2_BUCKET_NAME" "$BACKUP_DIR/$f" "$f" >/dev/null
            rm -f "$BACKUP_DIR/$f"
        done
